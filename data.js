...
DAY_LEN: 120,
DRYER_PRICES: [0,0,0,150,300],           // 동결건조기 5대: 3대 기본, 4·5대째 구매                               // 하루 = 120초
ENDING_DAY: 30,
...
ICE: {
  vanilla:      {name:'바닐라',            price:3,   col:'#fff2c9', taste:[1,0,0,0,0], job:null,
    desc:'그냥 바닐라. 우유는 매일 배달된다.', src:'기본', icon:{shape:'swirl',deco:'cherry'}},
  // 농사
  fruit:        {name:'과일 아이스크림',    price:8,   col:'#f28c28', taste:[2,0,0,0,0], job:'farm',
    desc:'밭에서 키운 과일. 동결건조해도 과일 맛은 남는다.', story:'유일하게 정상적으로 보이는 메뉴라 오히려 잘 안 팔린다.', src:'밭에 과일 씨앗', icon:{shape:'scoop',deco:'dots',col2:'#e84a5f'}},
  pumpkin:      {name:'호박 아이스크림',    price:12,  col:'#e0862a', taste:[1,1,0,0,0], job:'farm',
    desc:'통호박을 통째로 얼렸다. 숟가락이 안 들어간다.', story:'가을 한정이라고 써 붙였지만 사계절 판다.', src:'호박 씨앗 (농사 Lv2)', icon:{shape:'scoop',deco:'leaf'}},
  strawberry:   {name:'딸기 아이스크림',    price:15,  col:'#e84a5f', taste:[3,0,0,0,0], job:'farm',
    desc:'이 가게에서 가장 정상적인 메뉴. 아이들이 좋아한다.', story:'딸기는 원래 아이스크림이 되고 싶어 했다.', src:'딸기 씨앗 (농사 Lv4)', icon:{shape:'double',deco:'dots',col2:'#ffffff'}},
  fireberry:    {name:'불꽃 과일 아이스크림', price:90, col:'#ff5a1f', taste:[2,0,0,2,2], job:'farm',
    desc:'드래곤 피로 키운 과일. 얼어 있는데 뜨겁다.', story:'농부와 슬레이어가 손잡으면 이런 게 나온다.', src:'드래곤 피 비료 (교차 스킬)', icon:{shape:'scoop',deco:'flame'}},
  // 채집
  honey:        {name:'벌집 아이스크림',    price:15,  col:'#f4d35e', taste:[3,0,0,0,1], job:'gather',
    desc:'벌집째로 동결건조했다.', story:'벌은 대부분 뺐다. 대부분.', src:'숲의 벌집', icon:{shape:'scoop',deco:'drip',col2:'#e0a020'}},
  mushroom:     {name:'버섯 아이스크림',    price:12,  col:'#c9a27e', taste:[0,2,0,0,1], job:'gather',
    desc:'숲 버섯. 독버섯은 아니라고 한다.', story:'마녀가 단골이 되었다.', src:'숲 바닥의 버섯', icon:{shape:'scoop',deco:'dots',col2:'#ffffff'}},
  herb:         {name:'약초 아이스크림',    price:18,  col:'#7bbf6a', taste:[0,1,0,1,1], job:'gather',
    desc:'쓰다. 몸에 좋다고 하면 팔린다.', story:'약초 감식을 배우기 전엔 잡초로 보였다.', src:'숲의 약초 (채집 Lv2)', icon:{shape:'scoop',deco:'leaf'}},
  flower:       {name:'희귀 꽃 아이스크림',  price:40,  col:'#d98fe0', taste:[2,0,0,0,2], job:'gather',
    desc:'향이 강하다. 먹는 건지 꽂는 건지.', story:'꽃 감정사가 되어야 보이는 꽃.', src:'숲의 희귀 꽃 (채집 Lv4)', icon:{shape:'swirl',deco:'sparkle',col2:'#ffffff'}},
  spirit:       {name:'정령 열매 아이스크림', price:120, col:'#9fe8ff', taste:[3,1,0,1,3], job:'gather',
    desc:'먹으면 잠깐 정령이 보인다. 정령은 화나 있다.', story:'정령과 교감한 채집가만 딸 수 있다.', src:'깊은 숲의 정령 열매 (채집 Lv5)', icon:{shape:'swirl',deco:'sparkle',col2:'#ffffff',glow:true}},
  // 목축
  pork:         {name:'삼겹살 아이스크림',  price:25,  col:'#e58a8a', taste:[0,3,0,0,1], job:'ranch',
    desc:'동결건조한 삼겹살을 갈아 넣었다. 고소하다.', story:'돼지는 사흘이면 다 자란다. 사료에 뭐가 들었는지는 묻지 말자.', src:'돼지를 길러 도축', icon:{shape:'scoop',deco:'stripes'}},
  milk:         {name:'우유 아이스크림',    price:20,  col:'#fdfdf5', taste:[2,2,0,0,0], job:'ranch',
    desc:'그냥 우유 아이스크림. 이 가게에서 제일 이상한 메뉴 취급을 받는다.', story:'정상적인 게 제일 비정상.', src:'소의 우유 (목축 Lv3)', icon:{shape:'swirl',deco:'none'}},
  beef:         {name:'소고기 아이스크림',  price:35,  col:'#b8544a', taste:[0,3,0,1,1], job:'ranch',
    desc:'등급은 묻지 마라.', story:'소를 잡던 날 마을이 조용했다.', src:'소 도축', icon:{shape:'scoop',deco:'stripes'}},
  wool:         {name:'양털 아이스크림',    price:30,  col:'#eeeeee', taste:[0,0,0,0,3], job:'ranch',
    desc:'털을 얼렸다. 씹으면 씹힌다.', story:'양은 멀쩡하다. 털만 없다.', src:'양의 털 (목축 Lv4)', icon:{shape:'scoop',deco:'fluff'}},
  mutton:       {name:'양고기 아이스크림',  price:30,  col:'#c98a7a', taste:[0,2,0,1,1], job:'ranch',
    desc:'노린내는 얼려서 없앴다.', story:'양털 다음은 양이었다.', src:'양 도축', icon:{shape:'scoop',deco:'stripes'}},
  // 사냥
  rabbit:       {name:'토끼 아이스크림',    price:14,  col:'#d9c7b8', taste:[0,1,0,1,1], job:'hunt',
    desc:'귀엽던 것을 얼렸다.', story:'아이들에게는 "토끼 모양"이라고 한다.', src:'숲의 토끼', icon:{shape:'scoop',deco:'ears'}},
  bear:         {name:'곰발바닥 아이스크림', price:80,  col:'#6b4a3a', taste:[0,1,0,2,2], job:'hunt',
    desc:'동결건조한 곰발바닥. 발톱은 빼드립니다.', story:'전 주인은 이 곰 때문에 허리가 나갔다. 이제 곰은 아이스크림이다.', src:'숲의 곰', icon:{shape:'scoop',deco:'claw'}},
  wolf:         {name:'늑대 아이스크림',    price:60,  col:'#8a8f9a', taste:[0,1,0,3,1], job:'hunt',
    desc:'야성의 맛. 먹고 나면 달을 보고 싶어진다.', story:'산길을 개척하니 늑대가 있었다.', src:'산의 늑대 (사냥 Lv3)', icon:{shape:'scoop',deco:'claw'}},
  boar:         {name:'멧돼지 아이스크림',  price:55,  col:'#5f4b3a', taste:[0,2,0,2,1], job:'hunt',
    desc:'삼겹살의 야생 버전.', story:'조합장이 이걸 먹고 가게를 밀어주기로 했다.', src:'산의 멧돼지 (사냥 Lv3)', icon:{shape:'scoop',deco:'stripes'}},
  dragon_scale: {name:'드래곤 비늘 아이스크림', price:200, col:'#3fa07a', taste:[0,0,0,2,3], job:'hunt',
    desc:'바삭하다. 이가 나갈 수도 있다.', story:'용을 잡은 날, 가게 앞에 줄이 섰다.', src:'드래곤 해체', icon:{shape:'scoop',deco:'scales',glow:true}},
  dragon_heart: {name:'드래곤 심장 아이스크림', price:500, col:'#c9262b', taste:[0,2,0,3,3], job:'hunt',
    desc:'아직 뛰는 것 같다. 기분 탓이다.', story:'기사 롤랑이 평생 궁금해하던 맛. 그냥저냥이었다.', src:'드래곤 해체', icon:{shape:'scoop',deco:'heart',glow:true}},
  dragon_eye:   {name:'드래곤 눈알 아이스크림', price:300, col:'#f2c14e', taste:[0,0,1,1,3], job:'hunt',
    desc:'쳐다본다. 먹을 때까지.', story:'괴식파 손님들의 성지가 되었다.', src:'드래곤 해체', icon:{shape:'scoop',deco:'eye',glow:true}},
  dragon_wing:  {name:'드래곤 날개막 아이스크림', price:250, col:'#7a5cbf', taste:[0,1,0,3,2], job:'hunt',
    desc:'쫄깃하다. 하늘을 나는 꿈을 꾼다.', story:'해체 정밀 스킬이 있으면 더 많이 나온다.', src:'드래곤 해체', icon:{shape:'swirl',deco:'scales',glow:true}},
  // 낚시
  carp:         {name:'붕어 아이스크림',    price:10,  col:'#b58b5a', taste:[0,0,2,0,0], job:'fish',
    desc:'연못 붕어. 흙내가 난다.', story:'어부 만석이 낚싯대를 줬다.', src:'연못 낚시', icon:{shape:'scoop',deco:'fishtail'}},
  hairtail:     {name:'갈치구이 아이스크림', price:40,  col:'#b9c7d6', taste:[0,1,3,0,0], job:'fish',
    desc:'구운 갈치를 동결건조했다. 비린내는 얼려서 없앴다.', story:'바다에 나가니 갈치가 제철이었다.', src:'바다 낚시 (낚시 Lv2)', icon:{shape:'scoop',deco:'fishtail'}},
  marlin:       {name:'청새치 아이스크림',  price:120, col:'#4a78b8', taste:[0,2,2,1,0], job:'fish',
    desc:'노인과 바다. 아이스크림.', story:'대물 낚시를 배운 뒤 잡을 수 있었다.', src:'바다 낚시 (낚시 Lv4)', icon:{shape:'double',deco:'fishtail'}},
  seadragon:    {name:'수룡 아이스크림',    price:600, col:'#2fd3c8', taste:[0,0,3,3,3], job:'fish',
    desc:'바다의 용. 낚고 나서 싸워야 한다.', story:'낚시꾼이면서 사냥꾼이어야 한다.', src:'부두 심해 낚시 (낚시 Lv5 + 사냥 Lv4)', icon:{shape:'swirl',deco:'scales',glow:true}},
  // 어둠
  mystery:      {name:'??? 아이스크림',     price:150, col:'#3a2a4a', taste:[0,0,0,0,3], job:null,
    desc:'마녀가 준 수상한 재료로 만들었다. 묻지 않는 게 좋다.', story:'잘 팔린다. 그게 문제다.', src:'마녀 이바의 거래', dark:true, icon:{shape:'scoop',deco:'sparkle',col2:'#c05aa0',glow:true}},
},
ING_EXTRA: { feed:'사료' },
